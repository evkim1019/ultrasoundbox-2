...
import { getRenalBlocks } from '../store/blocks'
 ...
 * COMPONENT
 */
class RenalBlock extends Component {
  constructor(props) {
    super(props);
  ...
  async componentDidMount() {
    ...
    const thisBlock = this.props.renal.filter(block => block.id - 1 === parseInt(this.props.match.params.id))[0]
    ...
    this.setState({
      ...
      data: {
        userId: this.props.userId,
        cardioblockId: null,
        renalblockId: parseInt(this.props.match.params.id) + 1,
        efastblockId: null,
        ruqblockId: null,
        count: this.props.currentUserTakes.filter(take => take.renalblockId === parseInt(this.props.match.params.id) + 1).length,
        score: 0,
        wrong: []
      },
    })
  ...
  async submitAnswer(ev) {
    ...
    let currentTake = { score: score, wrong: wrong, count: 1, userId: this.props.userId, renalblockId: parseInt(this.props.match.params.id) + 1 }
    ...
    return (
      <div className="content-box" id="block">
        ...
        <div id="prop-box">
          ...
          <p className="smallnote">Renal / Block {this.props.match.params.id * 1 + 1}</p>
        ...
        <div className="float-right-bottom">
          <div>
            {this.props.renal.map(block => block.id).includes(this.props.match.params.id * 1 + 2) ?
              <a onClick={() => { window.location.href = `/renal/${this.props.match.params.id * 1 + 1}` }}>Next Block</a>
              :
...
const mapDispatchToProps = dispatch => {
  return {
    getRenalBlocks: () => dispatch(getRenalBlocks()),
...
export default connect(mapState, mapDispatchToProps)(RenalBlock)
